...
public class GoogleAccessTokenCacheService {
    static final String ACCESS_TOKEN_PREFIX = "google:accessToken:";
    private final StringRedisTemplate accessTokenRedisTemplate;
    private final EncryptionService encryptionService;

    public void addAccessToken(
            String email, String accessToken, long expiresIn, TimeUnit timeUnit) {
        String encryptedToken = encryptionService.encrypt(accessToken);
        accessTokenRedisTemplate
                .opsForValue()
                .set(ACCESS_TOKEN_PREFIX + email, encryptedToken, expiresIn, timeUnit);
    }

    public String getAccessToken(String email) {
        String accessToken =
                accessTokenRedisTemplate.opsForValue().get(ACCESS_TOKEN_PREFIX + email);
